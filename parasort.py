#!/usr/bin/env python3
"""
PARASORT - URL Parameter Categorization Tool for Penetration Testing

A tool that automatically categorizes URLs by their parameters for efficient vulnerability testing. 
...
class ParaSort:
    def __init__(self, config_file="parameter_categories.json"):
    ...
    def load_config(self):
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                print(f"{Fore.YELLOW}Created config: {self.config_file}")
    ...
    def process_urls(self, input_file, output_dir, categories_to_use, custom_params=None, 
                   clear_params=False, verbose=False, silent=False):
        """Process all URLs from input file and categorize them by domain"""
...
██║     ██║  ██║██║  ██║██║  ██║███████║╚██████╔╝██║  ██║   ██║   
╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝ ╚═════╝ ╚═╝  ╚═╝   ╚═╝   
{Fore.YELLOW}                    URL Parameter Categorizer
...
def main():
    parser = argparse.ArgumentParser(
        description=f"{Fore.WHITE}PARASORT - URL Parameter Categorization Tool for Penetration Testing",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        add_help=False,
        epilog=f"""
{Fore.CYAN}📋 {Fore.WHITE}EXAMPLES:{Style.RESET_ALL}
  {Fore.YELLOW}Basic usage:{Fore.WHITE}
    {sys.argv[0]} -i urls.txt -o results

  {Fore.YELLOW}Specific vulnerabilities:{Fore.WHITE}  
    {sys.argv[0]} -i urls.txt -o output --vuln sqli xss

  {Fore.YELLOW}Custom parameters:{Fore.WHITE}
    {sys.argv[0]} -i urls.txt -o output -cp "id,user,cmd"
    {sys.argv[0]} -i urls.txt -o output -cp id user cmd
    {sys.argv[0]} -i urls.txt -o output -f my_params.txt
    {sys.argv[0]} -i urls.txt -o output --clear -v
...
{Fore.GREEN}👤 {Fore.WHITE}Author: {Fore.YELLOW}Ev3rPalestine{Style.RESET_ALL}
    ...
    # Required arguments (only required for processing mode)
    ...
    required.add_argument('-i', '--input', metavar='FILE',
                        help=f'{Fore.WHITE}Input file containing URLs (e.g., -i urls.txt){Style.RESET_ALL}')
    required.add_argument('-o', '--output', default='results', metavar='DIR',
                        help=f'{Fore.WHITE}Output directory (e.g., -o results){Style.RESET_ALL}')
    ...
    custom_group.add_argument('-f', '--custom-params-file', metavar='FILE',
                            help=f'{Fore.WHITE}File with custom parameters (e.g., -f params.txt){Style.RESET_ALL}')
    
    # Information
    info = parser.add_argument_group(f'{Fore.CYAN}ℹ️ INFORMATION{Style.RESET_ALL}')
    info.add_argument('--show-categories', action='store_true',
                     help=f'{Fore.WHITE}List available vulnerability categories (e.g., --show-categories){Style.RESET_ALL}')
    ...
    if not args.input:
        print(f"{Fore.RED}Error: the following arguments are required: -i/--input")
        print(f"{Fore.YELLOW}Use {sys.argv[0]} --help for usage information")
        sys.exit(1)
